    ...
    def __getitem__(self, index):
        ...
        for neighbor in self.neighbor_list:
            if self.is_lmdb:
                ...
                path_3d = self.lq_root / clip / seq / f'im{neighbor}_40.png'
            img_bytes = self.file_client.get(img_lq_path, 'lq')
            bytes_3d = self.file_client.get(path_3d, 'lq')
            img_lq = mmcv.imfrombytes(img_bytes).astype(np.float32) / 255.
            img_3d = mmcv.imfrombytes(bytes_3d).astype(np.float32) / 255.
          #  img_con_3d = np.vstack((img_lq, img_3d))
            img_con_3d = np.concatenate((img_lq,img_3d),axis=2)
            ##visualization
            number +=1
            
            visual_lq = img_con_3d[:,:,:3]
            visual_lq = Image.fromarray((visual_lq*255.).astype(np.uint8)).convert("RGB")
            visual_lq.save(path+'/'+str(number)+'_lq.png')
            visual_3d = img_con_3d[:,:,3:]
            visual_3d = Image.fromarray((visual_3d*255.).astype(np.uint8)).convert("RGB")
            visual_3d.save(path+'/'+str(number)+'_3d.png')
            ##visualization
            img_lqs.append(img_con_3d)
